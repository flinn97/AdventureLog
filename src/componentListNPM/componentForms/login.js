	...
	async handleSubmission()  {
        ...
        if(user){
            this.props.app.dispatch({
                login:false, registerPage:false, user: this.props.app.state.componentList.getComponent("user"), email: this.state.email
            })
        }
    ...
    render(){
        ...
        return(
                    ...
                    borderWidth: styles?.mySpawn?.border, width: "18vw"}} type="password" id="pwd"   onChange={this.handleChange} name="password"/>
                        ...
                        <div>
                         <button style={{...styles?.buttons?.buttonFollow, marginTop:"2vh", fontSize: styles?.fonts?.fontHeader2,}} className= "btn" onClick={this.handleSubmission}>Login</button>
